...
public class SessionFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if (sessionCookie != null) {
			log.debug("Active session cookie = " + sessionCookie.getValue());
			String sessionId = sessionCookie.getValue();

			// retrieve session in db
			SessionDTO session = sessionService.getSessionBySessionId(sessionId);
			log.debug("Session from db: " + session.toString());
			...
			if (session.getAuthorities().isEmpty() && isGetRequest) {
				log.debug("Using public session -> Adding csrf-token into request attribute.");
		...
		log.debug("Created public session = " + newSession.getSessionId());
